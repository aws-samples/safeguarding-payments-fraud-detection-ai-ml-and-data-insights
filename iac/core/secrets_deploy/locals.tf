...
locals {
  ...
  secret = {
    ...
    SPF_SERVICE_AZ1       = element(data.aws_subnet.this.*.availability_zone, 0)
    SPF_SERVICE_AZ2       = element(data.aws_subnet.this.*.availability_zone, 1)
    SPF_SERVICE_AZ3       = element(data.aws_subnet.this.*.availability_zone, 2)
