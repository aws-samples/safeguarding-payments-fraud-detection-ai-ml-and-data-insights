...
resource "aws_subnet" "igw" {
  count                = var.subnets_igw_create ? length(values(local.igw_map)) : 0
...
resource "aws_internet_gateway" "igw" {
  count  = var.subnets_igw_create ? 1 : 0
  vpc_id = data.terraform_remote_state.sg.outputs.vpc_id
}
...
resource "aws_route_table_association" "igw" {
  count          = var.subnets_igw_create ? length(local.igw_ids) : 0
...
resource "aws_subnet" "nat" {
  count                = var.subnets_nat_create ? length(values(local.nat_map)) : 0
  vpc_id               = data.terraform_remote_state.sg.outputs.vpc_id
...
resource "aws_eip" "nat" {
  count  = var.subnets_nat_create ? 1 : 0
  domain = "vpc"
}
...
resource "aws_route_table_association" "nat" {
  count          = var.subnets_nat_create ? length(local.nat_ids) : 0
...
resource "aws_subnet" "cagw" {
  count                = var.subnets_cagw_create ? length(values(local.cagw_map)) : 0
  vpc_id               = data.terraform_remote_state.sg.outputs.vpc_id
...
resource "aws_ec2_carrier_gateway" "cagw" {
  count  = var.subnets_cagw_create ? 1 : 0
  vpc_id = data.terraform_remote_state.sg.outputs.vpc_id
}
...
resource "aws_route_table_association" "cagw" {
  count          = var.subnets_cagw_create ? length(local.cagw_ids) : 0
...
# resource "aws_subnet" "lgw" {
#   count                = var.subnets_lgw_create ? length(values(local.lgw_map)) : 0
#   vpc_id               = data.terraform_remote_state.sg.outputs.vpc_id
#   cidr_block           = element(values(local.lgw_map), count.index)
#   availability_zone_id = element(keys(local.lgw_map), count.index)
# }
